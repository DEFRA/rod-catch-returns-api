 ...
 */
@Slf4j
public class ClientRestTemplateErrorHandler extends StandardResponseErrorHandler implements ResponseErrorHandler {
    @Override
    protected void handleError(final ClientHttpResponse response, final HttpStatus statusCode) {
        log.error("Unexpected response from Microsoft Dynamics using client template." + toResponseLogMessage(response, statusCode));
