...
public abstract class AbstractConstraintValidator<A extends Annotation, T> implements ConstraintValidator<A, T> {
     ...
     *
     * @param entity  the {@link AbstractRestrictedEntity} to check
     * @param context the validator context
     * @return true if valid, false otherwise
     */
    protected boolean checkRestrictedEntity(final AbstractRestrictedEntity<?> entity, final String fieldName,
                                            final ConstraintValidatorContext context) {
        boolean valid = true;
        if (entity != null && entity.isInternal() && !hasAuthority(SecurityContextHolder.getContext().getAuthentication(), USE_INTERNAL)) {
            valid = handleError(context, fieldName.toUpperCase() + "_FORBIDDEN", fieldName);
        }
        return valid;
    }
     ...
     * @param context      the validator context
     * @param code         the error code uniquely identifying the validation error that occured
     * @param propertyName the name of the property which cause the constraint violation
     * @return always returns false (to signify a validation error)
     */
    protected final boolean handleError(final ConstraintValidatorContext context, final String code,
